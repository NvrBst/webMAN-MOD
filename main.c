...
#define WM_VERSION			"1.34.10 MOD"						// webMAN version
...
#define LINELEN2		512 // html games
...
static void make_fb_xml(char *myxml)
{
	...
	sprintf(myxml, "<?xml version=\"1.0\" encoding=\"UTF-8\"?>"
					...
					"<Query class=\"type:x-xmb/folder-pixmap\" key=\"mgames\" attr=\"mgames\" "
							"src=\"xmb://localhost%s#seg_mygames\"/>"
					"</Items>"
					"</View>"
					"</XMBML>", STR_MYGAMES, SUFIX2(profile), STR_LOADGAMES, MY_GAMES_XML);
...
//
						...
						)
					{
						...
						if(is_net)
						{
							...
							sprintf(tempstr, "<Table key=\"%04i\">"
											 ...
											 "<Pair key=\"module_action\"><String>http://%s/mount_ps3/net%i%s/%s?random=%i</String></Pair>"
											 "<Pair key=\"info\"><String>/net%i%s</String></Pair>"
											 "</Table>",
									key, icon, templn, local_ip, (f0-7), param, enc_dir_name, (int)pTick.tick, (f0-7), param);
		...
		//title_foundx:
								...
								sprintf(tempstr, "<Table key=\"%04i\">"
												 ...
												 "<Pair key=\"title\"><String>%s</String></Pair>"
												 "<Pair key=\"module_name\"><String>webbrowser_plugin</String></Pair>"
												 "<Pair key=\"module_action\"><String>http://%s/mount_ps3%s/%s?random=%i</String></Pair>"
												 "<Pair key=\"info\"><String>%s%s</String></Pair>"
												 "</Table>",
									key, icon, templn, local_ip, param, enc_dir_name, (int)pTick.tick, (f0==NTFS?(char*)"/ntfs/":param), (f0==NTFS?paths[f1]:""));
...
//
		...
		sprintf(templn, "<?xml version=\"1.0\" encoding=\"UTF-8\"?>"
						...
						"<Pair key=\"info\"><String>%s</String></Pair>"
						"<Pair key=\"module_name\"><String>webbrowser_plugin</String></Pair>"
						"<Pair key=\"module_action\"><String>http://%s/mount_ps3/unmount</String></Pair>"
						"</Table>", wm_icons[11], STR_EJECTDISC, STR_UNMOUNTGAME, local_ip); strcpy(myxml, templn);
		...
		if(!webman_config->noset)
		{
			sprintf(templn, "<Table key=\"setup\">"
							 ...
							 STR_WMSETUP, STR_WMSETUP2); strcat(myxml, templn);

			if(add_xmbm_plus)
				strcat(myxml, "<Pair key=\"child\"><String>segment</String></Pair></Table>");
			else
				{sprintf(templn, "<Pair key=\"module_action\"><String>http://%s/setup.ps3</String></Pair></Table>", local_ip); strcat(myxml, templn);}
		}
...
*/
					...
					else
					{
						...
						if(upd_time==0)
						{
							...
							t_line_entries;
							t_line_entries *line_entry	= (t_line_entries *)sysmem_html;
							u16 max_entries=((BUFFER_SIZE))/LINELEN2;
...
//
										...
										)
									{
										...
										if(is_net)
										{
											...
											strncpy(line_entry[idx].path, tempstr, LINELEN2); idx++;
											tlen+=strlen(tempstr);
											if(tlen>(BUFFER_SIZE-1024)) break;
										}
										else
...
#define PS3MAPI_SERVER_VERSION					0x0102
#define PS3MAPI_SERVER_MINVERSION				0x0102
...
static u32 BUFFER_SIZE_PS3MAPI = (64 * KB);
...
static void handleclient_ps3mapi(u64 conn_s_ps3mapi_p)
{
	...
	char buffer[2048];
	char cmd[16], param[384], param2[384];
	char param3[128], param4[256];

	u32 cpu_temp, rsx_temp;
	...
	#define PS3MAPI_CONNECT_NOTIF "PS3M_API new client: %s\r\n"
	...
	sprintf(buffer, PS3MAPI_OK_220, PS3MAPI_SERVER_VERSION); ssend(conn_s_ps3mapi, buffer);
	...
	while (connactive == 1 && working)
	{
		...
		if (recv(conn_s_ps3mapi, buffer, 2047, 0) > 0)
		{
			buffer[strcspn(buffer, "\n")] = '\0';
			buffer[strcspn(buffer, "\r")] = '\0';
			...
			else if (strcasecmp(cmd, "SERVER") == 0)
			{
				if (split == 1)
				{
					split = ssplit(param, cmd, 15, param2, 383);
			...
			else if (strcasecmp(cmd, "PS3") == 0)
			{
				if (split == 1)
				{
					...
					else if (strcasecmp(cmd, "GETFWTYPE") == 0)
					{
						char fw[28];
						memset(fw, 0, 28);
						{ system_call_3(8, SYSCALL8_OPCODE_PS3MAPI, PS3MAPI_OPCODE_GET_FW_TYPE, (u64)fw); }
					...
					else if (strcasecmp(cmd, "LED") == 0)
					{
						if (split == 1)
						{
							split = ssplit(param2, param3, 127, param4, 255);
							if (split == 1)
							{
								u64 color = val(param3);
								u64 mode = val(param4);
					...
					else if (strcasecmp(cmd, "FULLCLEAN") == 0)
					{
					...
					else if (strcasecmp(cmd, "DELHISTORYL") == 0)
					{
						delete_history(false);
						ssend(conn_s_ps3mapi, PS3MAPI_OK_200);
					}
			...
			else if (strcasecmp(cmd, "PROCESS") == 0)
			{
				if (split == 1)
				{
					...
					if (strcasecmp(cmd, "GETNAME") == 0)
					{
						...
						char name[30];
						memset(name, 0, 30);
						{system_call_4(8, SYSCALL8_OPCODE_PS3MAPI, PS3MAPI_OPCODE_GET_PROC_NAME_BY_PID, (u64)pid, (u64)name); }
						sprintf(buffer, "200 %s\r\n", name);
						ssend(conn_s_ps3mapi, buffer);
					}
					...
					if (strcasecmp(cmd, "GET") == 0)
					{
						if (data_s > 0)
						{
							if (split == 1)
							{
								...
								if (split == 1)
								{
									split = ssplit(param4, param, 127, param2, 255);
									if (split == 1)
									{
										u64 offset = val(param);
										int size = val(param2);
										u32 attached_pid = val(param3);
										int rr = -4;
										sys_addr_t sysmem = 0;
				...
				if (split == 1)
				{
					...
					if (strcasecmp(cmd, "GETNAME") == 0)
					{
						if (split == 1)
						{
							...
							{
								u32 pid = val(param3);
								s32 prxid = val(param4);
								char name[32];
								memset(name, 0, 32);
								{system_call_5(8, SYSCALL8_OPCODE_PS3MAPI, PS3MAPI_OPCODE_GET_PROC_MODULE_NAME, (u64)pid, (u64)prxid, (u64)name); }
								sprintf(buffer, "200 %s\r\n", name);
								ssend(conn_s_ps3mapi, buffer);
							}
							else ssend(conn_s_ps3mapi, PS3MAPI_ERROR_501);
					...
					else if (strcasecmp(cmd, "GETFILENAME") == 0)
					{
						if (split == 1)
						{
							...
							if (split == 1)
							{
								...
								char name[260];
								memset(name, 0, 260);
								{system_call_5(8, SYSCALL8_OPCODE_PS3MAPI, PS3MAPI_OPCODE_GET_PROC_MODULE_FILENAME, (u64)pid, (u64)prxid, (u64)name); }
								sprintf(buffer, "200 %s\r\n", name);
								ssend(conn_s_ps3mapi, buffer);
							}
							else ssend(conn_s_ps3mapi, PS3MAPI_ERROR_501);
					...
					else if (strcasecmp(cmd, "LOAD") == 0)
					{
						if (split == 1)
						{
							...
							{
								u32 pid = val(param3);
								{system_call_6(8, SYSCALL8_OPCODE_PS3MAPI, PS3MAPI_OPCODE_LOAD_PROC_MODULE, (u64)pid, (u64)param4, NULL, 0); }
								ssend(conn_s_ps3mapi, PS3MAPI_OK_200);
							}
							else ssend(conn_s_ps3mapi, PS3MAPI_ERROR_501);
						}
			...
			else if (strcasecmp(cmd, "TYPE") == 0)
			{
				...
				{
					ssend(conn_s_ps3mapi, PS3MAPI_OK_200);
					if (strcmp(param, "A") == 0) dataactive = 1;
					else if (strcmp(param, "I") == 0)  dataactive = 0;
					else  dataactive = 1;
				}
				else
				{
...
static void ps3mapi_thread(u64 arg)
{
	...
	else show_msg((char *)"PS3M_API Server not loaded!");
